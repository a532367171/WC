...
namespace WC.BLL
{
    public class MobilePage : Page
    {
        ...
        private void IsLoginUser()
        {
            ...
            {
                this.Context.Response.Redirect("/InfoTip/Operate_Redirect.aspx?type=3&url=" + absoluteUri);
            }
            else
            {
                    ...
                    if (text2.Contains("|"))
                    {
                        ...
                        if (Utils.IsNumber(string.Concat(HttpContext.Current.Application[this.GetUtt()])))
                        {
                            ...
                            if (num > 15 && num <= num2)
                            {
                                ...
                                if (num4 != this.JavascriptOpenDiv() && Utils.IsRandomHit(20))
                                {
                                    this.Context.Response.Write(string.Concat(new object[]
									{
										"<script>if(window.parent !=null){window.parent.location='/InfoTip/Operate_mReg.aspx?tips=",
										num3,
										"';}else{window.location='/InfoTip/Operate_mReg.aspx?tips=",
										num3,
										"';}</script>"
									}));
                                    return;
                            ...
                            if (num > num2 && Utils.IsRandomHit(8))
                            {
                                this.Context.Response.Write(string.Concat(new object[]
								{
									"<script>if(window.parent !=null){window.parent.location='/InfoTip/Operate_mReg.aspx?ns=",
									num2,
									"';}else{window.location='/InfoTip/Operate_mReg.aspx?ns=",
									num2,
									"';}</script>"
								}));
                                return;
