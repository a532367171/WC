...
public partial class Manage_Utils_SelectPeople_r : ViewPages

	{
		...
		{
			if (!string.IsNullOrEmpty(base.Request.QueryString["obj"]))
			{
				this.bj = base.Request.QueryString["obj"];
			}
			if (!base.IsPostBack)
			{
				this.InitData();
		...
		protected void Search_Btn(object sender, EventArgs e)
		{
			...
			string text2 = "?keywords=" + HttpContext.Current.Server.HtmlEncode(text.Trim()) + "&obj=" + base.Request.QueryString["obj"];
			if (!string.IsNullOrEmpty(base.Request.QueryString["v"]))
			{
				text2 = text2 + "&v=" + base.Request.QueryString["v"];
			}
			base.Response.Redirect("SelectPeople_r.aspx" + text2);
		}
		private void Show()
		{
