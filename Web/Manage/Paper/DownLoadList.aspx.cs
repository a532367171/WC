...
public partial class Manage_Paper_DownLoadList : ModulePages
{
		...
		private void Show()
		{
			...
			int num3 = num * (num2 - 1);
			int num4 = Convert.ToInt32(MsSqlOperate.ExecuteScalar(CommandType.Text, "select count(*) from PaperDownload ", null));
			this.num.InnerHtml = "当前查询条件总计 - <span style='color:#ff0000; font-weight:bold;'>" + num4 + "</span> 条 记录数据";
			string sql = "select top " + num + " * from PaperDownload  order by id desc";
			if (num2 != 1)
			{
				sql = string.Concat(new object[]
				{
					...
					num3.ToString(),
					" id FROM PaperDownload ORDER BY id DESC) T1) ORDER BY id DESC"
				});
			}
