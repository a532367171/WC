...
<div class="Block">
    <div class="Possition">
		<img src="<% = ForumConfig.ReadConfigInfo().SkinPath %>/boardicon.gif" align="absmiddle" /><DXBBS:Possition ID="Possition1" runat="server" />发表新主题
	</div>
</div>
<div class="Block Hidden">
    ...
    <div class="Middle M1 Hidden">
        ...
        <div class="TopicRight P5"><asp:TextBox ID="Color" Text="#FFFFFF" runat="server" /> <span id="ColorSpan" style="background-color:#000000">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>&nbsp;<IMG style="CURSOR: pointer" onclick="SelectColor('_ctl0:Main:Post1:_ctl0:Color','ColorSpan')" src="images/forecolor.gif"></div>
    </div>
    </asp:PlaceHolder>
	...
	<div class="Middle M1 Hidden">
	    ...
	    <div class="TopicRight P5"><iframe src="Upload.aspx?BoardID=<% = Requests.GetIntQueryString("BoardID") %>&TopicID=<% = Requests.GetIntQueryString("TopicID") %>&ReplyID=<% = Requests.GetIntQueryString("ReplyID") %>&Types=0" width="100%" height="22px" frameborder="0" allowTransparency="true" scrolling="no"></iframe></div>
	</div>
    <div class="Middle M1 Hidden">
		<div class="TopicLeft">
			...
			<asp:Repeater ID="Emote" runat="server">
			    <itemTemplate>
			    <img src="<%# Eval("FileName") %>" title="<%# Eval("EmoteName") %>" style="cursor:pointer;padding:2px;height:18px;width:18px" onclick="Emote('<%# Eval("FileName") %>','_ctl0:Main:Post1:_ctl0:Content')" />
			    </itemTemplate>
			</asp:Repeater>
			</div>
		</div>
    ...
    <asp:PlaceHolder ID="ShowTopicCode" runat="server">
    <div class="Middle M1 Hidden">
        <div class="TopicLeft P5">验证码：</div>
        <div class="TopicRight P5"><asp:TextBox ID="TopicCode" Width="60px" runat="server" /> <asp:RequiredFieldValidator ID="NeedCode" ErrorMessage="请输入验证码" ControlToValidate="TopicCode" Display="dynamic" runat="server" /><img src="ShowCode.aspx" align="absmiddle" /></div>
    </div>
	</asp:PlaceHolder>
	<div class="Middle TC" <% if (Sessions.UserID == 0) { %> style='display:none;' <%}%> ><asp:Button CssClass="Button" ID="PostButton" Text="确定发表" runat="server" /></div>
</div>