...
<div class="BodyBlock PTB5">
	<div class="Block Hidden">
		...
		<dl class="Middle TC">
			...
			<dd class="TL"><a href='Admin_NewsAdd.aspx?ID=<%# Eval("ID") %>'><%# Eval("Title") %></a></dd>
		</dl>
		</ItemTemplate>
		</asp:Repeater>
		<div class="Footer PTB5 TC">
			<input type="button" class="Button" value="添加公告" onclick="window.location.href='Admin_NewsAdd.aspx'" />&nbsp;<asp:Button CssClass="Button" ID="DeleteButton" Text="确定删除" OnClientClick="return CheckSelect()" runat="server" />&nbsp;<input type="checkbox" name="All" onclick="SelectAll(this)" />全选/全不选
		</div>
	</div>
